 ...
 * @param {function} done
 */
function apiCallback(headerToken, done, apikeys) {
  let test = false;

  if (apikeys && apikeys.length > 0) {
    test = apikeys.indexOf(headerToken) > -1;
  } else {
    test = headerToken === process.env.CLAY_ACCESS_KEY;
  }
  if (test) {
    ...
    done(null, false, { message: 'Unknown apikey: ' + headerToken });
  }
}
 ...
 * @param {Array} apikeys
 */
function createAPIKeyStrategy(apikeys) {
  const APIKeyStrategy = require('passport-http-header-token').Strategy;

  passport.use('apikey', new APIKeyStrategy({}, (headerToken, done) => apiCallback(headerToken, done, apikeys)));
}
 ...
 * @returns {Function}
 */
function createStrategy(site, apikeys) {
  return function (provider) {
    switch (provider) {
      ...
      case 'ldap': createLDAPStrategy(site);
        ...
        break;
      case 'apikey': createAPIKeyStrategy(apikeys);
 ...
 * @returns {array}
 */
function init(router, providers, site, sessionStore, apikeys) {
  ...
  _.each(providers, module.exports.createStrategy(site, apikeys)); // allow mocking this in tests
