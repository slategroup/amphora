...
function getAuthUrl(site) {
  ...
  var base = references.uriToUrl(site.prefix, site.proto, site.port);
 ...
 * @returns {Promise}
 */
function verify(properties, site) {
  return function (req, token, tokenSecret, profile, done) { // eslint-disable-line
    ...
    // get UID
    uid = site.prefix + '/users/' + users.encode(`${username.toLowerCase()}`, provider);
