...
const path = require('path'),
  ...
  layouts = require('./services/layouts'),
...
function applyPrefix(data, site) {
  ...
  //applyPrefixToLayouts(bootstrap, site);
...
function load(data, site) {
  let promiseComponentsOps, promiseLayoutsOps, promisePagesOps, promiseUrisOps, promiseListsOps, promiseUsersOps,
    prefix = site.prefix,
  ...
  promiseLayoutsOps = saveWithInstances(`${prefix}/_layouts/`, compData._layouts, function (name, item) {
    return bluebird.join(
      dbOps.getPutOperations(layouts.put, name, _.cloneDeep(item), locals.getDefaultLocals(site))
    ).then(_.flatten);
  });
...
function bootstrapComponents(site) {
  return bluebird.all(_.map(files.getComponents().concat(files.getLayouts()), function (component) {
    const componentPath = files.getComponentPath(component) || files.getLayoutPath(component);
