...
function resolveComponentReferences(data, locals, filter = referenceProperty) {
  // n.b. these are references to the objects in place in the data, so updating each one will update the source
  const referenceObjects = references.listDeepObjects(data, filter);

  return Promise.all(
    referenceObjects.map((referenceObject) => {
      return components
        .get(referenceObject[referenceProperty], locals)
        .then((obj) => {
          // the thing we got back might have its own references
          return resolveComponentReferences(obj, locals, filter)
            .finally(() => {
...
function filterBaseInstanceReferences(obj) {
  return (
    _.isString(obj[referenceProperty]) &&
    obj[referenceProperty].indexOf("/instances/") !== -1
  );
}
