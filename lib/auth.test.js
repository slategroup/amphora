...
describe(_.startCase(filename), function () {
  ...
  describe('deserializeUser', function () {
    ...
    it('calls `done` with the user if one is returned', function () {
      var done = sinon.spy(),
    ...
    it('calls `done` error if one is found', function () {
      var done = sinon.spy(),
        userError = new Error('user not found');

      db.get.returns(Promise.reject(userError));
      fn('person', done)
        .then(function () {
          sinon.assert.calledOnce(done);
          sinon.assert.calledWith(db.get, '/users/person');
  ...
  describe('apiCallback', function () {
    ...
    it('allows api key that matches CLAY_ACCESS_KEY', function (done) {
      ...
      fn('123', function (err, data) {
        expect(err).to.equal(null);
        expect(data).to.deep.equal({ provider: 'apikey', auth: 'admin' });
        process.env.CLAY_ACCESS_KEY = oldKey;
        done();
      });
    });

    it('allows api key that matches ALT_CLAY_ACCESS_KEY', function (done) {
      var oldKey = process.env.ALT_CLAY_ACCESS_KEY;

      process.env.ALT_CLAY_ACCESS_KEY = '123';
      fn('123', function (err, data) {
        expect(err).to.equal(null);
        expect(data).to.deep.equal({ provider: 'apikey', auth: 'admin' });
        process.env.ALT_CLAY_ACCESS_KEY = oldKey;
        done();
      });
    });
  ...
  describe('init', function () {
    ...
    it('mounts session stuff to router', function () {
      ...
      expect(router.use.callCount).to.equal(6);
    });
  ...
  describe('withAuthLevel', function () {
    ...
    it('sends unauthorized response if user does not have proper permissions', function () {
      ...
      sinon.assert.calledOnce(responses.unauthorized);
    });
    ...
    it('calls next if the user has the appropriate level', function () {
      ...
      sinon.assert.notCalled(responses.unauthorized);
    });
  });
});
